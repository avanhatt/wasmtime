...
impl SolverCtx {
    ...
    fn extract_equal_symbolic(
        ...
        let mut some_match = vec![];
        let mut ite_str = self.smt.eq(source1, source2);

        // Special case: if we are asked to extract the bitwidth, just return the source
        let matching = self.smt.eq(self.smt.numeral(self.bitwidth), symbolic_high);
        some_match.push(matching);

        // Possible amounts to extract by
        for possible_high in vec![1, 8, 16, 32] {
            // Statement meaning the symbolic case matches this concrete case
            let matching = self.smt.eq(self.smt.numeral(possible_high), symbolic_high);
            some_match.push(matching);
            let extract_eq = self.smt.eq(
                self.smt.extract(possible_high, 0, source1),
                self.smt.extract(possible_high, 0, source2),
            );
            ite_str = self.smt.ite(matching, extract_eq, ite_str);
        }
        let some_shift_matches = self.smt.or_many(some_match);
        self.width_assumptions.push(some_shift_matches);
        ite_str
...
pub fn run_solver(rule_sem: RuleSemantics, query_width: usize) -> VerificationResult {
    ...
    let side_equality = match width {
        Some(w) => {
            ...
            let lhs_care_bits =
                ctx.smt
                    .extract(w.checked_sub(1).unwrap().try_into().unwrap(), 0, lhs);
            let rhs_care_bits =
                ctx.smt
                    .extract(w.checked_sub(1).unwrap().try_into().unwrap(), 0, rhs);

            ctx.smt.eq(lhs_care_bits, rhs_care_bits)
        }
        None => {
            ...
            ctx.additional_assumptions
                .push(ctx.smt.eq(lhs_width, rhs_width));
