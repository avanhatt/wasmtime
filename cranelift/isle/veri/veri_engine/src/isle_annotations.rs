...
use veri_ir::{VIRType, VIRAnnotation};
use veri_ir::annotation_ir::{BoundVar, Function, TermSignature, TermAnnotation, Expr, Type};
...
pub fn isle_annotation_for_term(term: &str) -> Option<TermAnnotation> {
    match term {
        ...
        "lower" | "put_in_reg" | "value_reg" | "first_result" | "inst_data" => {
            // No-op for now
            let arg = BoundVar::new("arg", &Type::BitVector);
